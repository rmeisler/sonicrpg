...
return function(self, target)
	...
	return Serial {
		...
		Do(function()
			...
			if self.releaseVal > 15 then
				self.scene.audio:playSfx("levelup")
			elseif self.releaseVal > 8 then
		...
		Do(function()
			...
			if self.releaseVal > 15 then
				stats.attack = stats.attack * 1.8 -- good
			elseif self.releaseVal > 8 then
				stats.attack = stats.attack * 1.2 -- mid
			end
			Executor(self.scene):act(target:takeDamage(stats))
		end),
		Parallel {
