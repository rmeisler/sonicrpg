...
return {
	...
	behavior = function (self, target)
		if math.random() < 0.5 then
			...
			return Serial {
				Telegraph(self, "Double Shot", {255,255,255,50}),
				Animate(self.sprite, "shoot"),
				Animate(self.sprite, "shoot_idle"),
				laserShot(target),
				laserShot(secondTarget),
				Animate(self.sprite, "shoot_retract"),
				Animate(self.sprite, "idle")
			}
		else
			local targetActionList = {}
			local poisonStats = table.clone(self.stats)
			poisonStats.speed = 100
			...
			return Serial {
				...
				Do(function()
					...
					self.gasSprite.transform.y = self.sprite.transform.y - 50 + 7
				end),
				Parallel {
